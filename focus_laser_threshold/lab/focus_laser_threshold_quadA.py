...
# Config JSON path
json_config_path = '../ascanio/config' #../../../config/FocusLaser'
json_config_fname = 'focus_laser_config_phasespace_xy_asca.json'
...
# Adaptive X step sizes
xstart = focuslaser_jsonconfig['global_params']['xstart_um']
xstop = focuslaser_jsonconfig['global_params']['xstop_um']
xstep_big = focuslaser_jsonconfig['global_params']['xstep_big_um'] 
xstep_fine = focuslaser_jsonconfig['global_params']['xstep_fine_um']
x_threshold_start = focuslaser_jsonconfig['global_params']['x_threshold_start_um']
x_threshold_stop = focuslaser_jsonconfig['global_params']['x_threshold_stop_um']

# Generate x_array with adaptive steps
x_array = []
x_current = xstart
...
ystop = focuslaser_jsonconfig['global_params']['ystop_um']
ystep = focuslaser_jsonconfig['global_params']['ystep_um']
step_y = int((ystop - ystart) / ystep) + 1
y_array = np.linspace(ystart, ystop, step_y)
...
# Data save location
save_dir_base = pathlib.Path('{}/{}/GapScan/{}_{}_{}_{}_{}_{}_250617_focus_laser_thres_LB1471C_quadA'.format(
    data_save_root, 
...
save_fname_str = "{}_{}_{}_{}_{}_{}_{}_QPDGapScan_KnifeEdge_250617_focus_laser_thres_LB1471C_quadA".format(
    focuslaser_jsonconfig['dut']['batch'],
    focuslaser_jsonconfig['dut']['foundry'],
    focuslaser_jsonconfig['dut']['wafer'],
    focuslaser_jsonconfig['dut']['dut_type'],
...
for y in y_array:
    ...
    save_dir_str = save_dir_base / f"Y{y:04.0f}um"
    if not save_dir_str.exists():
        save_dir_str.mkdir(parents=True, exist_ok=True)

    # Reset Z position
    zpos = focuslaser_jsonconfig['global_params']['zstart_um']
    stage.conex.move(lta00, 1, float(zpos), 'abs')

    while zpos >= zstop:
        zstep = zstep_fine if z_threshold_start >= zpos >= z_threshold_stop else zstep_big
        ...
        data = {
            'rawdata': {
                ...
                'stage_laser_zposition': np.array([zpos]),
                'quadA': {
        ...
        for idx, x in enumerate(x_array):
            ...
            # Measure DC photocurrent for Quadrant A with numavg single measurements
            try:
                navg = focuslaser_jsonconfig['global_params']['number_averages']
                temp_measurements = np.zeros(navg)
                for i in range(navg):
                    try:
                        dmm00.write("INIT")
                        time.sleep(0.05)
                ...
                print(f"Y={y} um, Z={zpos} um, X={x} um, DC photocurrent = {np.mean(temp_measurements):.7f} A")
            except Exception as e:
                print(f"Error measuring at Y={y} um, Z={zpos} um, X={x} um: {e}")
        ...
        plt.legend()
        plt.title(f'DC Photocurrent at Y={y} um, Z={zpos:.3f} mm')
