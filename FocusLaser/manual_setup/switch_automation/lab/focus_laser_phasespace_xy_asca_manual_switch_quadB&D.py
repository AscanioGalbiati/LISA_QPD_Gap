...
# Functions to switch connections for OUT2 (assuming DMM connected to OUT2)
...
def connect_quadB_out1(ser):  # IN4 to OUT1 for Quadrant B
    ser.write(b':open all\n')
    time.sleep(0.1)
...
# Data save location
save_dir_base = pathlib.Path('{}/{}/GapScan/{}_{}_{}_{}_{}_{}_250916_LB1471C_quadB_and_D_manual_setup_fixedY'.format(
    data_save_root, 
...
save_fname_str = "{}_{}_{}_{}_{}_{}_{}_QPDGapScan_KnifeEdge_250916_LB1471C_quadB_and_D_manual_setup_fixedY".format(
    focuslaser_jsonconfig['dut']['batch'],
    focuslaser_jsonconfig['dut']['foundry'],
    focuslaser_jsonconfig['dut']['wafer'],
    focuslaser_jsonconfig['dut']['dut_type'],
...
for x in x_array:
    ...
    # Create subfolder for this X position
    save_dir_str = save_dir_base / f"X{x*1000:04.0f}um"
    ...
    data = {
        'rawdata': {
            'comp_gps_time': np.zeros_like(y_array),
            'stage_laser_xposition': np.zeros_like(y_array),
            'stage_laser_yposition': np.zeros_like(y_array),
            'quadD': {
                'dmm00_curr_amp': np.zeros((len(y_array), focuslaser_jsonconfig['global_params']['number_averages']))
            },
            'quadB': {
                'dmm00_curr_amp': np.zeros((len(y_array), focuslaser_jsonconfig['global_params']['number_averages']))
            }
    ...
    for idx, y in enumerate(y_array):
        ...
        # Measure DC photocurrent for Quadrant B (IN3 to OUT1)
        try:
            print(f"\nSwitching matrix to Quadrant B (IN3 to OUT1)...")
            connect_quadB_out1(ser)
            ...
            for i in range(navg):
                try:
                    ...
                    print(f"Error in single measurement {i+1} at X={x} mm, Y={y} mm for Quadrant B: {e}")
                    temp_measurements_b[i] = np.nan
            data['rawdata']['quadB']['dmm00_curr_amp'][idx] = temp_measurements_b
            print(f"X={x} mm, Y={y} mm, DC photocurrent Quadrant B = {np.mean(temp_measurements_b):.7f} A")
        except Exception as e:
            print(f"Error measuring at X={x} mm, Y={y} mm for Quadrant B: {e}")
            data['rawdata']['quadB']['dmm00_curr_amp'][idx] = np.nan
        
        # Measure DC photocurrent for Quadrant D (IN4 to OUT1)
        try:
            print(f"\nSwitching matrix to Quadrant D (IN4 to OUT1)...")
            ...
            print(f"X={x} mm, Y={y} mm, DC photocurrent Quadrant D = {np.mean(temp_measurements_d):.7f} A")
        except Exception as e:
            print(f"Error measuring at X={x} mm, Y={y} mm for Quadrant D: {e}")
            data['rawdata']['quadD']['dmm00_curr_amp'][idx] = np.nan

    print(f"DC photocurrent Quadrant C = {np.mean(temp_measurements_b)}")
    print(f"DC photocurrent Quadrant D = {np.mean(temp_measurements_d)}")

    # Save data
    fout = '{}_{}_{}_X{:04d}um_Z{:05d}um'.format(
        data['rawdata']['start_date'],
        data['rawdata']['start_time'],
        save_fname_str,
        int(np.round(x, decimals=3) * 1000),
    ...
    # Plot data
    plt.plot(y_array, data['rawdata']['quadB']['dmm00_curr_amp'].mean(axis=1), label='quadB')
    plt.plot(y_array, data['rawdata']['quadD']['dmm00_curr_amp'].mean(axis=1), label='quadD')
    plt.legend()
    plt.title(f'DC Photocurrent at X={x} mm')
