...
json_config_fname = 'focus_laser_config_phasespace_xy_asca_manual_allQuadrants.json'
...
# Data save location
save_dir_base = pathlib.Path('{}/{}/GapScan/{}_{}_{}_{}_{}_{}_251001_LB1471C_quadABCD_manual_setup_Z50mm_Yscan_gap_identification_precise'.format(
    data_save_root, 
...
save_fname_str = "{}_{}_{}_{}_{}_{}_{}_QPDGapScan_KnifeEdge_251001_LB1471C_quadABCD_manual_setup_Z50mm_Yscan_gap_identification_precise".format(
    focuslaser_jsonconfig['dut']['batch'],
    focuslaser_jsonconfig['dut']['foundry'],
    focuslaser_jsonconfig['dut']['wafer'],
    focuslaser_jsonconfig['dut']['dut_type'],
...
for y in y_array:
    ...
    for idx, x in enumerate(x_array):
        ...
        # Measure DC photocurrent for Quadrant A
        try:
        ...
        # Measure DC photocurrent for Quadrant B
        try:
            ...
            print(f"Y={y} mm, X={x} mm, DC photocurrent Quadrant B = {np.mean(temp_measurements_b):.7f} A")
        except Exception as e:
            print(f"Error measuring at Y={y} mm, X={x} mm for Quadrant B: {e}")
            data['rawdata']['quadB']['dmm00_curr_amp'][idx] = np.nan
            
        # Measure DC photocurrent for Quadrant C
        try:
            ...
            for i in range(navg):
                try:
                    ...
                    print(f"Error in single measurement {i+1} at X={x} mm, Y={y} mm for Quadrant C: {e}")
                    temp_measurements_c[i] = np.nan
            data['rawdata']['quadC']['dmm00_curr_amp'][idx] = temp_measurements_c
            print(f"Y={y} mm, X={x} mm, DC photocurrent Quadrant C = {np.mean(temp_measurements_c):.7f} A")
        except Exception as e:
            print(f"Error measuring at Y={y} mm, X={x} mm for Quadrant C: {e}")
        ...
        try:
            ...
            print(f"Y={y} mm, X={x} mm, DC photocurrent Quadrant D = {np.mean(temp_measurements_d):.7f} A")
        except Exception as e:
    ...
    plt.plot(x_array, data['rawdata']['quadB']['dmm00_curr_amp'].mean(axis=1), label='quadB')
    plt.plot(x_array, data['rawdata']['quadC']['dmm00_curr_amp'].mean(axis=1), label='quadC')
