...
ax10.set_xlabel(r'Translatin Stage Z Position [mm]', fontsize=14)
...
for y_pos in procdata:
    ...
    if procdata[y_pos][segn]['z_positions'] and procdata[y_pos][segn]['beamwaist_dc']:
        ...
        ax10.plot(filtered_z, filtered_w0, linestyle=linestyle, color=line_color, alpha=0.5, label=segn)
...
ax10.plot(z_min, w_min, 'o', mec='red', mfc='none', mew=2.5, ms=12)

# ---- FIXED LaTeX inset label -------------------------------------------------
tex_label = r'$w_{{0,\min}} = {:.2f}\,\mu\mathrm{{m}}$'.format(w_min)
result_line = Line2D([0], [0], marker='o', color='w',
                     markeredgecolor='red', markerfacecolor='none',
                     markersize=10, markeredgewidth=2.5,
                     label=tex_label)
inset = ax10.inset_axes([0.39, 0.6, 0.35, 0.25])
inset.axis('off')
inset.legend(handles=[result_line], loc='upper left', fontsize=14, frameon=False, handlelength=1.2)

# Legend: larger font, shifted right
ax10.legend(fontsize=13, loc='upper left', frameon=True, bbox_to_anchor=(0.12, 0.975))

# ----- Custom tick formatter: 0 → "0", others → 1 decimal -----
def tick_formatter(val, pos):
    return '0' if val == 0 else f'{val:.3f}'
y_fmt = FuncFormatter(tick_formatter)
...
ax.set_xlabel(r'Translation Stage X Position [mm]', fontsize=14)
...
cax = fig.add_axes([0.12, 0.695, 0.02, 0.18])  # [left, bottom, width, height]
# Colormap bar
cbar = fig.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=cmap), cax=cax, orientation='vertical')
cbar.set_label(r'Z Position [mm]', fontsize=12, labelpad=10) #=8
...
# Small line legend for first/last Z
