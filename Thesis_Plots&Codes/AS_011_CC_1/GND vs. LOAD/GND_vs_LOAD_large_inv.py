...
for y_um, y_folder in y_folders:
    print(f"Y = {y_um} µm")
    fig_dir = os.path.join(y_folder, "fig_GND_LOAD")
    ...
    # Purple sums
    line_gnd, = ax.plot(x_f, sum_gnd, '--', color='#992f7f', lw=2, label='quadA+quadD (LOAD)')
    line_load,  = ax.plot(x_f, sum_load,  '--', color='#992f7f', lw=2, alpha=0.6, label='quadA+quadD (GND)')
    ...
    ax.set_title(rf'\textbf{{DC Photocurrent GND vs. LOAD (HL: Y={y_um}\textmu{{}}m)}}', fontsize=14, pad=10)
