'''
Horizontal data analysis – FINAL 3σ uncertainty | ZEROS ALIGNED | QE in BROWN | WITH GAP OVERSHOOT BOX
...
for y_um, y_folder in y_positions:
    ...
    ax_load.plot(x_array_filtered, I_sum_load, '--', color="#992f7f", linewidth=2.0, label='quadB+quadC', alpha=0.6)
    ...
    ax_qe_load.plot(x_array_filtered, QE_load, color="#00ff88", linewidth=2, alpha=0.2, label=r'$\eta_{\rm{LOAD}}$ [A/W]') # Fixed label from GND
    ...
    inset_load.axis('off')
    legend_handle = Line2D([0], [0], color=(*plt.cm.colors.to_rgb('#992f7f'), 0.6), lw=2, ls='--',
    ...
    ax_gnd.plot(x_array_filtered, I_sum_gnd, '--', color="#992f7f", linewidth=2.0, alpha=1.0, label='quadB+quadC')
    ...
    ax_qe_gnd.plot(x_array_filtered, QE_gnd, color="#44fd00", linewidth=2, label=r'$\eta_{\rm{GND}}$ [A/W]', alpha=0.2) # Fixed label
