# -*- coding: utf-8 -*-
"""
DIAGONAL Knife-Edge Scan – Gap AD (quadrants A & D)
...
# Point this at a single Along folder, or the parent directory if you want to loop over multiple Along folders
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/all quadrants/20251130/VIGO_FPW01_QPD_1500_20_B17R11_251126_LB1471C_quadABCD_manual_setup_Z13.66mm_LINESCAN_Gap_AD"
...
calc_min = 0.19
calc_max = 0.45
left_region_default  = (calc_min, 0.27)   # left region (start of the plot region)
right_region_default = (0.37, calc_max)  # right region (end of the plot region)
gap_region_default   = (0.27, 0.37)  # region to search for over/undershoot
...
# ====================== LASER POWER – use the same values as BC reference ======================
...
for along_folder in along_folders:
    ...
    def process_file(pkl_path, kind_label, current_regions, u_min_plot, u_max_plot, current_results):
        ...
        # --- Plotting Data ---
        ax.plot(u_plot, quadA, label='quadA', color='tab:blue', lw=1.8)
        ax.plot(u_plot, quadD, label='quadD', color='tab:red', lw=1.8) # quadD is traditionally red
        ax.plot(u_plot, I_sum, '--', color="#992f7f", lw=2.0, alpha=0.7, label='quadA+quadD')

        # --- Reference Lines ---
        #ax.axhline(mean_plat, color='gray', ls=':', lw=1.5, label=r'Region Mean $I_{\rm{Plat}}$')
        ax.axhline(I_eta100, color="#c39d7a", linestyle='--', linewidth=1.8, alpha=0.95, label=r'$\eta = 1\,$A/W')
        ax.axhline(I_eta080, color='#8b5a2b', linestyle='--', linewidth=1.8, alpha=0.95, label=r'$\eta = 0.8\,$A/W')

        # --- QE Axis ---
        QE = I_sum / P_combined
        ax_qe = ax.twinx()
        qe_color = "#00ff88" if kind_label == "LOAD" else "#44fd00"
        ax_qe.plot(u_plot, QE, color=qe_color, linewidth=2, alpha=0.22, label=rf'$\eta_{{{kind_label}}}$ [A/W]')
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent {kind_label} – Gap AD (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
        ...
        preferred_order = [
            'quadA', 'quadD', 'quadA+quadD', # AD specific quadrants
            r'Left Region', r'Right Region', r'Gap Region', r'Region Mean $I_{\rm{Plat}}$',
            r'$\eta = 1\,$A/W', r'$\eta = 0.8\,$A/W', rf'$\eta_{{{kind_label}}}$ [A/W]'
        ]
...
print("\nAll AD diagonal scans processed – complete with AB-style features!")