...
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/all quadrants/20251130/VIGO_FPW01_QPD_1500_20_B17R11_251126_LB1471C_quadABCD_manual_setup_Z13.66mm_LINESCAN_Gap_AB"
...
calc_min = -0.4
calc_max = -0.1
left_region_y2  = (calc_min , -0.28) 
right_region_y2 = (-0.18, calc_max)
gap_region_y2   = (-0.28, -0.18) 
...
for along_folder in along_folders:
    ...
    if load_files:
        ...
        # --- Plotting Data ---
        ax.plot(u_plot, quadA_load, label='quadA', color='tab:blue', lw=1.8)
        ax.plot(u_plot, quadB_load, label='quadB', color='tab:orange', lw=1.8)
        ax.plot(u_plot, I_sum_load, '--', color="#992f7f", lw=2.0, alpha=0.7, label='quadA+quadB')
        ...
        #inset = ax.inset_axes([0.32, 0.54, 0.38, 0.16], transform=ax.transAxes)
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent LOAD – Gap AB (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
    ...
    if gnd_files:
        ...
        # --- Plotting Data ---
        ax.plot(u_plot, quadA_gnd, label='quadA', color='tab:blue', lw=1.8)
        ax.plot(u_plot, quadB_gnd, label='quadB', color='tab:orange', lw=1.8)
        ax.plot(u_plot, I_sum_gnd, '--', color="#992f7f", lw=2.0, label='quadA+quadB')
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent GND – Gap AB (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
        ax.set_xlabel(r'Perpendicular offset $u$ [mm]', fontsize=14)
        ax.set_ylabel(r'Photocurrent [A]', fontsize=14)
        ax.grid(True, ls='--', alpha=0.6)
        ax.tick_params(axis='both', which='major', labelsize=10, length=6, width=1.5, direction='in')
    ...
    if load_files or gnd_files:
        ...
        ordered_results = {
            ...
            'Mean_region_GND_µA': current_results['Mean_region_LOAD_µA'], # NOTE: Fixed typo in original, should be mean_region_GND_µA
        }
        # Re-set correct key for Mean_region_GND_µA in case of previous typo
        ordered_results['Mean_region_GND_µA'] = current_results['Mean_region_GND_µA'] 
