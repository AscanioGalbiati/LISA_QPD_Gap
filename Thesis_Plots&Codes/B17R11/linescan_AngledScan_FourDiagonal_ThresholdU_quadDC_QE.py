# -*- coding: utf-8 -*-
"""
DIAGONAL Knife-Edge Scan – Gap DC (quadrants D & C)
...
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/all quadrants/20251130/VIGO_FPW01_QPD_1500_20_B17R11_251126_LB1471C_quadABCD_manual_setup_Z13.66mm_LINESCAN_Gap_DC"
...
for along_folder in along_folders:
    ...
    def process_file(pkl_path, kind_label, current_regions, u_min_plot, u_max_plot):
        ...
        # Quadrant plots
        ax.plot(u_plot, quadD, label='quadD', color='tab:red',   lw=1.8) # D is red (opposite A/top)
        ax.plot(u_plot, quadC, label='quadC', color='tab:green', lw=1.8) # C is green (opposite B/right)
        ax.plot(u_plot, I_sum, '--', color="#992f7f", alpha=0.7, lw=2.5, label='quadD+quadC')
        ...
        ax_qe.plot(u_plot, QE, color=qe_color, lw=2, alpha=0.22, label=rf'$\eta_{{{kind_label}}}$ [A/W]')
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent {kind_label} – Gap DC (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
        ...
        # Define preferred order (adjusting for DC quadrants)
        preferred_order = [
...
print("\nAll DC diagonal scans processed – now fully optimized and consistent with the entire diagonal scan suite!")