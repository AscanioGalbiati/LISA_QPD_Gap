'''
Comparison of QPD (quad-A) KE beam waist vs. Slit-Scanner data with Z-shifts and ERF-fit
...
# USER SETTINGS 
# ----------------------------------------------------------------------
QPD_DIR = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20250617/VIGO17_NS089008_QPD_0750_20_AS_015_CC_asca_250617_focus_laser_thres_LB1471C_quadA/Y6100um"
...
SLIT_DIR = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20251021_BP"
...
# --- FONT SIZE & TICK SIZE UPDATES ---
ax.set_xlabel(r'Z Position [mm]', fontsize=16) # UPDATED FROM 14
ax.set_ylabel(r'Beam Radius $w$(z) [\textmu{}m]', fontsize=16) # UPDATED FROM 14
ax.set_title(r'\textbf{Beam Profile: KE (quadA) vs. BP Data}', fontsize=16, fontweight='bold') # UPDATED FROM 14
ax.set_xlim(46.6, 52.8) # 42 , 56
ax.set_ylim(bottom=-20, top=650) # 42 , 56
ax.grid(True, ls='--', alpha=0.6)
ax.tick_params(axis='both', which='major', labelsize=13, length=6, width=1.5, direction='in') # UPDATED FROM 10

# ---- QPD points (colour by SHIFTED Z) ----
for z_shifted, w in zip(qpd_z_shifted, qpd_w):
    ax.plot(z_shifted, w, 'o', color=color_dict[z_shifted], markersize=9) # MARKERSIZE 7 (Match Ref)
qpd_line, = ax.plot(qpd_z_shifted, qpd_w, '-', color='blue', lw=2.2, alpha=0.5, label=r'$w_0$(z) quadA (KE)') # LINEWIDTH 2.2

# ---- Slit data (already shifted & masked) ----
ax.plot(slit_z, slit_wx, 's', mfc='none', mec='black', ms=9, # MARKERSIZE 8 (Match Ref)
        label=r'$w_{\rm{{x}}}$(z) (BP)')
ax.plot(slit_z, slit_wy, '^', mfc='none', mec='black', ms=9, # MARKERSIZE 8 (Match Ref)
        label=r'$w_{\rm{{y}}}$(z) (BP)')
...
if len(z_left_x) > 1 and not np.isnan(z0_x):
    ...
    ax.plot(zl, wl, '--', color='gray', lw=2.0) # LINEWIDTH 2.0 (Match Ref style)
    ax.plot(zr, wr, '--', color='gray', lw=2.0, label=r'Linear fit: $w_{\rm{{x}}}$(z) (BP)') # LINEWIDTH 2.0
if len(z_left_y) > 1 and not np.isnan(z0_y):
    ...
    ax.plot(zl, wl, ':', color='dimgray', lw=2.0) # LINEWIDTH 2.0 (Match Ref style)
    ax.plot(zr, wr, ':', color='dimgray', lw=2.0, label=r'Linear fit: $w_{\rm{{y}}}$(z) (BP)') # LINEWIDTH 2.0

# ---- Intersection waists (red markers) ----
if not np.isnan(z0_x):
    ax.plot(z0_x, w0_x, 's', mec='red', mfc='none', mew=2.0, ms=13.5) # MEW 2.0, MS 10 (Match Slit style)
if not np.isnan(z0_y):
    ax.plot(z0_y, w0_y, '^', mec='red', mfc='none', mew=2.0, ms=13.5) # MEW 2.0, MS 10 (Match Slit style)
...
if qpd_w.size > 0:
    ...
    # Red open circle
    ax.plot(z_min_qpd, w_min_qpd, 'o', mec='red', mfc='none', mew=2.5, ms=13.5) # MS 12, MEW 2.5 (Match Ref)

    # --- Inset results box with QPD + Slit X + Slit Y ---
    inset = ax.inset_axes([0.34, 0.4, 0.38, 0.35])
    inset.axis('off')
    handles_inset = []
    ...
    if not np.isnan(z0_x):
        handles_inset.append(Line2D([0], [0], marker='s', color='w',
                                    markeredgecolor='red', markerfacecolor='none',
                                    markersize=10.5, markeredgewidth=2.0,
                                    label=rf'$w_{{0,\rm{{x}}}}$(BP) = ${w0_x:.2f}\,\mu\mathrm{{m}}$'))
    ...
    if not np.isnan(z0_y):
        handles_inset.append(Line2D([0], [0], marker='^', color='w',
                                    markeredgecolor='red', markerfacecolor='none',
                                    markersize=10.5, markeredgewidth=2.0,
                                    label=rf'$w_{{0,\rm{{y}}}}$(BP) = ${w0_y:.2f}\,\mu\mathrm{{m}}$'))

    inset.legend(handles=handles_inset, loc='upper left', fontsize=18.7, frameon=False, # UPDATED FROM 13 to 14 (closer to Ref 18.7)
                 handlelength=1.2, borderpad=0.8, labelspacing=0.8)

# ---- Legend (remove duplicates) ----
handles, labels = ax.get_legend_handles_labels()
by_label = dict(zip(labels, handles))
ax.legend(by_label.values(), by_label.keys(), fontsize=15, # UPDATED FROM 13 to 15 (Match Ref)
