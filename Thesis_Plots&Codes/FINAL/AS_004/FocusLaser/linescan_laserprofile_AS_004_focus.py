'''
Plots: 1) Beam Waist w(z) for quadB & quadC (KE); 2) Raw Photocurrent Traces for quadB & quadC.
ERF knife-edge fits for QPD .pkl data
'''
...
QPD_DIR = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20251105/Y2780"
...
QPD_Z_MIN_MM = 6.42 # allow reasonable range
QPD_Z_MAX_MM = 7.57
...
OUT_DIR = os.path.join(QPD_DIR, "fig")
...
ax_w.set_ylabel(r'Beam Waist $w_0$ [\textmu{}m]', fontsize=16)
...
if qpd_z_shifted_B.size:
    for z_s, w in zip(qpd_z_shifted_B, qpd_w_um_B):
        ax_w.plot(z_s, w, 'o', color=color_for_z(z_s), markersize=9)
    ax_w.plot(qpd_z_shifted_B, qpd_w_um_B, '--', color='orange', lw=2.2, alpha=0.8, label=r'quadB')

if qpd_z_shifted_C.size:
    for z_s, w in zip(qpd_z_shifted_C, qpd_w_um_C):
        ax_w.plot(z_s, w, 'D', color=color_for_z(z_s), markersize=9)
    ax_w.plot(qpd_z_shifted_C, qpd_w_um_C, '-.', color='green', lw=2.2, alpha=0.8, label=r'quadC')
...
if qpd_w_um_B.size:
    ...
    ax_w.plot(z_min_B, w_min_B, 'o', mec='red', mfc='none', mew=2.5, ms=13.5)
...
if qpd_w_um_C.size:
    ...
    ax_w.plot(z_min_C, w_min_C, 'D', mec='red', mfc='none', mew=2.5, ms=13.5)
    min_info.append(("C", w_min_C, z_min_C))

# Inset box with values
if min_info:
    inset = ax_w.inset_axes([0.3, 0.48, 0.45, 0.4])
    inset.axis('off')
    handles = []
    labels = []
    for tag, wv, zv in min_info:
        ...
        handles.append(Line2D([0], [0], marker=marker, color='w', markeredgecolor=mcol,
                              markerfacecolor='none', markersize=10.5, markeredgewidth=2.0))
        labels.append(label)
    try:
...
ax_w.legend(fontsize=15, loc='upper left', frameon=True, bbox_to_anchor=(0.09, 0.975))

# Axis limits
all_z_values = np.concatenate([qpd_z_shifted_B, qpd_z_shifted_C])
if all_z_values.size:
    ax_w.set_xlim(np.min(all_z_values), np.max(all_z_values))
...
out_path_w = os.path.join(OUT_DIR, "BeamWaist_KE_quadB_quadC.png")
...
for z_shift, x_arr, dc in qpd_data_collection_B:
    ...
    ax_curr.plot(x_arr / 1000.0, dc, '--', color=color, linewidth=2.2, alpha=0.75)
...
ax_curr.set_title(r'\textbf{DC Photocurrent (Y=2780\textmu{}m, quadB \& quadC)}', fontsize=18, fontweight='bold', pad=10)
...
ax_curr.set_xlim(6.9-0.02, 7.7+0.005)
...
if unique_z:
    ...
    cax = fig_curr.add_axes([0.15, 0.3, 0.02, 0.15])
...
# Legend for quadB/quadC line style difference
legend_handles = [
    Line2D([0], [0], linestyle='--', color='gray', linewidth=2.5),
    Line2D([0], [0], linestyle='-.', color='gray', linewidth=2.5)
]
#ax_curr.legend(handles=legend_handles, fontsize=14, loc='upper right', frameon=True)


out_path_curr = os.path.join(OUT_DIR, "Photocurrent_Traces_quadB_quadC.png")
