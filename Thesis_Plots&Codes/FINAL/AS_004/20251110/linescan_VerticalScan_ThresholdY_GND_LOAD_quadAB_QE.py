...
# Base directory
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20251110/VIGO_NS089008_QPD_1000_20_AS_004_objLens_quadABCD_manual_setup_GND_LOAD_251107_Z7.0_NDfilter/VerticalScan"

# === PLOT RANGE (narrow – beautiful figures) ===
# Default plot range
y_min_plot_default = 2.42
y_max_plot_default = 2.605

# Specific plot range for the second processed file (Index 1)
y_min_plot_y2 = 2.27
y_max_plot_y2 = 2.45
# Initialize current plot range with default
y_min_plot = y_min_plot_default
y_max_plot = y_max_plot_default

# === DYNAMIC REGION DEFINITIONS (1/2) ===
# Define default (or first file) regions
calc_min_default = 2.1
calc_max_default = 2.7
left_region_default  = (calc_min_default, 2.50)   # left plateau
right_region_default = (2.525, calc_max_default)   # right plateau
gap_region_default   = (2.50, 2.525)   # gap

# Define specific regions for the second file (Example: X=7300 µm, or index 1)
calc_min_y2 = 2.25
calc_max_y2 = 2.48
left_region_y2  = (calc_min_y2, 2.32) 
right_region_y2 = (2.4, calc_max_y2)
gap_region_y2   = (2.32, 2.4) 
# ============================================
...
csv_dir = "/Users/asca/Documents/University/Master Thesis/code/LISA_QPD_Gap/Thesis_Plots&Codes/FINAL/AS_004/20251110/statistical study"
os.makedirs(csv_dir, exist_ok=True)
csv_path = os.path.join(csv_dir, "Overshoot_Results_AB_X_up_to_7300um.csv")
...
for x_um, x_folder in x_positions:
    ...
    if x_index == 0:
        # First processed file (uses defaults)
        current_regions = {
        ...
        # For the second scan, use the specific regions and the NEW PLOT RANGE
        current_regions = {
            'calc_min': calc_min_y2, 'calc_max': calc_max_y2,
            'left': left_region_y2, 'right': right_region_y2, 'gap': gap_region_y2
        }
        y_min_plot = y_min_plot_y2
        y_max_plot = y_max_plot_y2
    else:
        # Revert to default for subsequent scans
        current_regions = {
            'calc_min': calc_min_default, 'calc_max': calc_max_default,
            'left': left_region_default, 'right': right_region_default, 'gap': gap_region_default
        }
    ...
    mask_plot = (y_array >= y_min_plot) & (y_array <= y_max_plot) # Uses dynamic y_min/max_plot
    ...
    # Uses dynamic y_min/max_plot
    ax_load.set_xlim(y_min_plot + (0.02 if x_index != 2 else 0.02), y_max_plot - (0.02 if x_index != 2 else 0.02))
    ...
    ax_gnd.axvspan(current_regions['right'][0], current_regions['right'][1], color='tab:orange', alpha=0.12, label='Right region')
    ...
    # GND Inset (Raised to match LOAD: [0.32, 0.78, 0.38, 0.16])
    inset_g = ax_gnd.inset_axes([0.32, 0.78, 0.38, 0.16], transform=ax_gnd.transAxes)
    inset_g.axis('off')
    # <<< INSET MARKER ADJUSTMENT: lw=3.2 -> 2.2, mew=2.5 -> 1.8, ms=12 -> 10, handlelength=2.5 -> 1.8 >>>
    ...
    # Uses dynamic y_min/max_plot
    ax_gnd.set_xlim(y_min_plot + (0.02 if x_index != 2 else 0.02), y_max_plot - (0.02 if x_index != 2 else 0.02))
...
print(f"\n=== ALL DONE (VERTICAL SCAN – QUADRANTS A+B) ===\n"
      f"Overshoot results (X $\leq$ 7200 $\mu$m) successfully saved to:\n{csv_path}")
