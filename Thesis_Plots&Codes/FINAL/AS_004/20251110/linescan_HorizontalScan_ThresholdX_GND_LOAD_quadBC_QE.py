...
# Base directory
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20251110/VIGO_NS089008_QPD_1000_20_AS_004_objLens_quadABCD_manual_setup_GND_LOAD_251107_Z7.0_NDfilter/HorizontalScan"
...
# =======================================================
# === DYNAMIC REGION DEFINITIONS (1/2) - CUSTOMIZED FOR Y2000, Y2200, Y2400 ===
...
Y_POS_CONFIG = {
    2800: {
        # Custom range for Y2400
        'x_min_plot': 7.15, 
        'x_max_plot': 7.45,
        'left': (calc_min_default, 7.24),
...
def y_formatter_qe(val, pos):
    # This formatter ensures the QE labels are formatted as requested (e.g., '0.8', '1.0')
...
for y_um, y_folder in y_positions:
    ...
    # The following line is replaced/overridden, so it is commented for clarity:
    ...
    ax_qe_load.set_ylim(bottom=0, top=1.05) # Ensure top limit is set before alignment
    
    align_yaxis_zeros(ax_load, ax_qe_load)
    
    # *** FIX: Force fixed ticks and re-apply formatter after alignment ***
    # This ensures the ticks (and their labels) land exactly on 0.8 and 1.0
    ax_qe_load.set_yticks(np.arange(0, 1.01, 0.2)) 
    ...
    ax_qe_gnd.set_ylim(bottom=0, top=1.05) # Ensure top limit is set before alignment
    
    align_yaxis_zeros(ax_gnd, ax_qe_gnd)
    
    # *** FIX: Force fixed ticks and re-apply formatter after alignment ***
    # This ensures the ticks (and their labels) land exactly on 0.8 and 1.0
    ax_qe_gnd.set_yticks(np.arange(0, 1.01, 0.2))
