'''
@author: A_Galbiati
Comparison of focusing results using the KE tecnhique and BP data fitted with a linear model
Linear fit on far-field BP data and theoretical waist estimate
'''
...
from matplotlib.ticker import FuncFormatter # Added for potential future use or consistency
...
ax.set_title(r'\textbf{Beam Profile: KE (quadB \& quadC) vs. BP Data (Linear Fit)}', fontsize=18, fontweight='bold') # FONTSIZE 18
...
if slit_z_mm_masked.size:
    if 'fit_left_x' in locals() and not np.isnan(z0_x):
        ...
        if zl.size: ax.plot(zl, wl, '--', color='gray', lw=1.8) # LW=1.8
        if zr.size: ax.plot(zr, wr, '--', color='gray', lw=1.8, label=r'Linear fit: $w_{\rm{{x}}}$(z) (BP)') # LW=1.8
    if 'fit_left_y' in locals() and not np.isnan(z0_y):
        ...
        if zl.size: ax.plot(zl, wl, ':', color='dimgray', lw=1.8) # LW=1.8
        if zr.size: ax.plot(zr, wr, ':', color='dimgray', lw=1.8, label=r'Linear fit: $w_{\rm{{y}}}$(z) (BP)') # LW=1.8

# plot slit intersection waists
if not np.isnan(z0_x):
    ax.plot(z0_x, w0_x_um, 's', mec='red', mfc='none', mew=3, ms=12) # MS=12, MEW=3
if not np.isnan(z0_y):
    ax.plot(z0_y, w0_y_um, '^', mec='red', mfc='none', mew=3, ms=12) # MS=12, MEW=3
...
if min_info:
    ...
    for tag, wv, zv in min_info:
        ...
        labels.append(label)
    try:
        inset.legend(handles=handles, labels=labels, loc='upper left', fontsize=16.8, frameon=False) # FONTSIZE 18
    except Exception:
        txt = "\n".join(labels)
...
if min_info:
    ...
    # Define a list of tags that are considered 'KE results'
    ke_tags = ["B", "C"]
    
    for i, (tag, wv, zv) in enumerate(min_info):
        # 1. Determine Marker and Label
        if tag == "B":
            marker = 'o'; mcol = 'red'; label = rf'$w_{{0,\min}}$(KE, quadB) = ${wv:.2f}\,\mu\mathrm{{m}}$'
        elif tag == "C":
            # Note: Removed the existing '\n' from the label for cleaner separation
            marker = 'D'; mcol = 'red'; label = rf'$w_{{0,\min}}$(KE, quadC) = ${wv:.2f}\,\mu\mathrm{{m}}$' 
        elif tag == "BP_x":
            marker = 's'; mcol = 'red'; label = rf'$w_{{0,\rm{{x}}}}$(BP) = ${wv:.2f}\,\mu\mathrm{{m}}$'
        else: # Assumed to be BP_y or similar
            marker = '^'; mcol = 'red'; label = rf'$w_{{0,\rm{{y}}}}$(BP) = ${wv:.2f}\,\mu\mathrm{{m}}$'
        
        # 2. Append the current item
        handles.append(Line2D([0], [0], marker=marker, color='w', markeredgecolor=mcol,
                              markerfacecolor='none', markersize=10, markeredgewidth=2.0))
        labels.append(label)
        
        # 3. INSERT SEPARATOR LOGIC
        # If this is the tag 'C' (the last KE tag), and there are more items to follow (i < len(min_info) - 1),
        # add a blank separator line.
        if tag == "C" and i < len(min_info) - 1:
            handles.append(Line2D([0], [0], marker='', linestyle='None'))
            labels.append(' ') # Add a space label to ensure balance
...
ax.legend(by_label.values(), by_label.keys(), fontsize=14, loc='upper left', frameon=True, bbox_to_anchor=(0.02, 0.98)) # FONTSIZE 14
...
# Save and show
out_path = os.path.join(OUT_DIR, "merged_QPD_vs_BP_comparison_styled.png")
