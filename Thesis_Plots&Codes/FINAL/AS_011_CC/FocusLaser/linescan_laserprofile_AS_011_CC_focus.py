...
QPD_Z_MIN_MM = 6.0 # allow reasonable range
QPD_Z_MAX_MM = 9.0
...
OUT_DIR = os.path.join(QPD_DIR, "fig_refactored")
...
ax_w.legend(fontsize=15, loc='upper left', frameon=True, bbox_to_anchor=(0.08, 0.975))

# Axis limits
all_z_values = np.concatenate([qpd_z_shifted_B, qpd_z_shifted_C])
if all_z_values.size:
    ax_w.set_xlim(np.min(all_z_values) -0.2, np.max(all_z_values) + 0.2)
...
out_path_w = os.path.join(OUT_DIR, "BeamWaist_KE_quadB_quadC_refactored.png")
...
for z_shift, x_arr, dc in qpd_data_collection_B:
    ...
    ax_curr.plot(x_arr / 1000.0, dc, '--', color=color, linewidth=2.2, alpha=1.0)
...
ax_curr.set_title(r'\textbf{DC Photocurrent (Y=6100\textmu{}m, quadB \& quadC)}', fontsize=18, fontweight='bold', pad=10)
...
out_path_curr = os.path.join(OUT_DIR, "Photocurrent_Traces_quadB_quadC_refactored.png")
