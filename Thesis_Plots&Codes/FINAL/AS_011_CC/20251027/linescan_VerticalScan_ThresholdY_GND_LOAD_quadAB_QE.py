...
# 1. DEFINE NEW FINAL OUTPUT DIRECTORY (Same as previous task)
...
for x_um, x_folder in x_positions:
    ...
    ax_load.plot(y_plot, quadA_load[mask_plot], label='quadA', color='tab:blue', linewidth=2.1)
    ax_load.plot(y_plot, quadB_load[mask_plot], label='quadB', color='tab:orange', linewidth=2.1)
    ...
    fig_load.savefig(os.path.join(FINAL_FIG_DIR, fname_load), dpi=300, bbox_inches='tight')
    print(f"LOAD saved: {os.path.join(FINAL_FIG_DIR, fname_load)}")
    plt.show()
    plt.close(fig_load)
    ...
    # Increased line width: 2.1
    ax_gnd.plot(y_plot, quadA_gnd[mask_plot], label='quadA', color='tab:blue', linewidth=2.1)
    ax_gnd.plot(y_plot, quadB_gnd[mask_plot], label='quadB', color='tab:orange', linewidth=2.1)
    ...
    fig_gnd.savefig(os.path.join(FINAL_FIG_DIR, fname_gnd), dpi=300, bbox_inches='tight')
    print(f"GND saved : {os.path.join(FINAL_FIG_DIR, fname_gnd)}")
    plt.show()
    plt.close(fig_gnd)
