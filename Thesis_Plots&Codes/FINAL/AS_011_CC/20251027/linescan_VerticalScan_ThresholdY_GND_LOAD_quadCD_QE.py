...
# === USER SETTINGS ===
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/20251215_final_data/20251027/VIGO_NS089008_QPD_750_20_AS_0011_CC_objLens_quadABCD_manual_setup_GND_LOAD_XY/VerticalScan"
...
# 1. DEFINE NEW FINAL OUTPUT DIRECTORY (Same as previous tasks)
...
for x_um, x_folder in x_positions:
    ...
    # DELETED: fig_dir creation using x_folder (now using FINAL_FIG_DIR)
    ...
    # Plotting (Increased line width: 2.1)
    ax_load.plot(y_arr, quadC_load_plot, label='quadC', color='tab:green', linewidth=2.1)
    ax_load.plot(y_arr, quadD_load_plot, label='quadD', color='tab:red', linewidth=2.1)
    ...
    # 4. UPDATED SAVE PATH FOR LOAD FIGURE
    ...
    # Increased line width: 2.1
    ax_gnd.plot(y_arr, quadC_gnd_plot, label='quadC', color='tab:green', linewidth=2.1)
    ax_gnd.plot(y_arr, quadD_gnd_plot, label='quadD', color='tab:red', linewidth=2.1)
    ...
    # Increased font size: 16
    # Increased tick label size: 13
    ax_qe_g.set_ylabel(r'Responsivity $\eta$ [A/W]', color='#8b5a2b', fontsize=16)
    ...
    # Result box GND (Adjusted position: [0.32, 0.78, 0.38, 0.16])
    ...
    # 4. UPDATED SAVE PATH FOR GND FIGURE
