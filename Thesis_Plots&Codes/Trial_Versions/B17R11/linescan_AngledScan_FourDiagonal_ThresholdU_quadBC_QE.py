# -*- coding: utf-8 -*-
"""
DIAGONAL Knife-Edge Scan – Gap BC (quadrants B & C)
...
base_dir = "/Users/asca/Documents/University/Master Thesis/code/Data/all quadrants/20251130/VIGO_FPW01_QPD_1500_20_B17R11_251126_LB1471C_quadABCD_manual_setup_Z13.66mm_LINESCAN_Gap_BC"
...
u_min_plot_default = 0.2
u_max_plot_default = 0.4

# Region definitions for the FIRST Y position
calc_min = 0.12 # take this from config
calc_max = 0.4
left_region_default  = (calc_min, 0.32)   # left region
right_region_default = (0.38, calc_max)   # right region
gap_region_default   = (0.32, 0.38)   # gap region
...
for along_folder in along_folders:
    ...
    fig_dir = os.path.join(along_folder, "fig_B17R11")
    ...
    if load_files:
        ...
        ax.plot(u_plot, quadB_load, label='quadB', color='tab:orange', lw=1.8)
        ax.plot(u_plot, quadC_load, label='quadC', color='tab:green',   lw=1.8)
        ax.plot(u_plot, I_sum_load, '--', color="#992f7f", lw=2.5, alpha=0.7, label='quadB+quadC')

        # --- Reference Lines ---
        #ax.axhline(mean_plat_load, color='gray', ls=':', lw=1.5, label=r'Region Mean $I_{\rm{Plat}}$')
        ax.axhline(I_eta100, color="#c39d7a", ls='--', lw=1.8, alpha=0.95, label=r'$\eta = 1\,$A/W')
        ax.axhline(I_eta080, color='#8b5a2b', ls='--', lw=1.8, alpha=0.95, label=r'$\eta = 0.8\,$A/W')

        QE_load = I_sum_load / P_combined
        ax_qe = ax.twinx()
        ax_qe.plot(u_plot, QE_load, color="#00ff88", lw=2, alpha=0.2, label=r'$\eta_{\rm{LOAD}}$ [A/W]')
        ...
        ax.plot(u_ext_load, val_load, 'o', color=marker_color,
                mec=mec_color, mew=1.8, ms=10, zorder=10)

        # --- Inset with GS Terminology ---
        inset = ax.inset_axes([0.09, 0.43, 0.38, 0.16], transform=ax.transAxes)
        inset.axis('off')
        handle = Line2D([0], [0], color="#992f7f", lw=2.5, ls='--',
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent LOAD – Gap BC (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
        ...
        # Define preferred order (adjusting for BC quadrants)
        preferred_order = [
            'quadB', 'quadC', 'quadB+quadC',
            r'Left Region', r'Right Region', r'Gap Region', r'Region Mean $I_{\rm{Plat}}$',
            r'$\eta = 1\,$A/W', r'$\eta = 0.8\,$A/W', r'$\eta_{\rm{LOAD}}$ [A/W]'
        ]
        ...
        ax.legend(final_handles, final_labels, 
                  fontsize=11, loc='upper right', bbox_to_anchor=(0.98, 0.98), 
    ...
    if gnd_files:
        ...
        ax.plot(u_plot, quadB_gnd, label='quadB', color='tab:orange', lw=1.8)
        ax.plot(u_plot, quadC_gnd, label='quadC', color='tab:green',   lw=1.8)
        ax.plot(u_plot, I_sum_gnd, '--', color="#992f7f", lw=2.0, label='quadB+quadC')
        ...
        ax_qe = ax.twinx()
        ax_qe.plot(u_plot, QE_gnd, color="#44fd00", lw=2, alpha=0.22, label=r'$\eta_{\rm{GND}}$ [A/W]')
        ax_qe.set_ylabel(r'Quantum Efficiency $\eta$ [A/W]', color='#8b5a2b', fontsize=14)
        ax_qe.tick_params(axis='y', colors='#8b5a2b', length=8, width=1.5, direction='in')
        ax_qe.yaxis.set_major_formatter(qe_formatter)
        ax_qe.set_ylim(0, 1.05)
        ...
        # --- Final Plot Styling ---
        ax.set_title(rf'\textbf{{DC Photocurrent GND – Gap BC (Y={y_um}\,\textmu{{m}})}}', fontsize=14, fontweight='bold', pad=10)
        ax.set_xlabel(r'Perpendicular offset $u$ [mm]', fontsize=14)
        ax.set_ylabel(r'Photocurrent [A]', fontsize=14)
        ax.grid(True, ls='--', alpha=0.6)
        ax.tick_params(axis='both', which='major', labelsize=10, length=6, width=1.5, direction='in')
...
print("\nAll BC diagonal scans processed – now fully optimized and consistent with your AB and AD scripts!")