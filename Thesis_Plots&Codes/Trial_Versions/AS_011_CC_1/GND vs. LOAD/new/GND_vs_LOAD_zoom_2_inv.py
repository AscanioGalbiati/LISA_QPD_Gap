...
using the robust plateau definition from the wider scan for accurate overshoot calculation.
...
# === FIGURE SAVE PATH ===
FINAL_FIG_DIR = '/Users/asca/Documents/University/Master Thesis/code/AS_011_CC/GND vs. LOAD/fig'
...
for y_um, folder in sorted([(y_pos(os.path.basename(f)), f)
    ...
    inset = ax.inset_axes([0.28, 0.70, 0.42, 0.20], transform=ax.transAxes)
    inset.axis('off')
    ...
    ax.legend(handles_clean, labels_clean, loc='upper right', fontsize=14, frameon=True, fancybox=False, edgecolor='black')
