...
calc_max = 7.45
left_region = (7.10, 7.255)
right_region = (7.275, 7.45)
gap_region = (7.255, 7.275)
...
for y_um, y_folder in y_positions:
    ...
    ax_load.plot(x_array_filtered, quadA_mean_load[mask_plot], label='quadA', color='tab:blue')
    ax_load.plot(x_array_filtered, quadD_mean_load[mask_plot], label='quadD', color='tab:red')
    ax_load.plot(x_array_filtered, I_sum_load_plot, '--', color="#992f7f", linewidth=2.0, label='quadA+quadD', alpha=0.6)
    
    # Colored regions
    ax_load.axvspan(7.10, 7.20, color='tab:blue', alpha=0.12, label='Left plateau')
    ax_load.axvspan(7.35, 7.45, color='tab:red', alpha=0.12, label='Right plateau')
    ...
    ax_qe_load.plot(x_array_filtered, QE_load, color="#00ff88", lw=2.0, ls='-', alpha=0.15, label=r'$\eta_{\rm{LOAD}}$ [A/W]')
    ...
    ax_load.set_xlim(x_min_plot+0.002, x_max_plot-0.002)
    ...
    ax_gnd.plot(x_array_filtered, quadA_mean_gnd[mask_plot], label='quadA', color='tab:blue')
    ax_gnd.plot(x_array_filtered, quadD_mean_gnd[mask_plot], label='quadD', color='tab:red')
    ax_gnd.plot(x_array_filtered, I_sum_gnd_plot, '--', color="#992f7f", linewidth=2.0, alpha=1.0, label='quadA+quadD')
    ...
    ax_qe_gnd.plot(x_array_filtered, QE_gnd, color="#44fd00", lw=2.0, alpha=0.15, label=r'$\eta_{\rm{GND}}$ [A/W]')
    ...
    ax_gnd.set_title(rf'\textbf{{DC Photocurrent GND (Y={y_um}\textmu{{}}m)}}', fontsize=14, fontweight='bold', pad=10)
    ...
    ax_gnd.set_ylim(bottom=0)
    ax_gnd.set_xlim(x_min_plot+0.002, x_max_plot-0.002)
...
'''
Changes to save the figures to a final folder

# ... (Before the main loop 'for y_um, y_folder in y_positions:')

# New fixed directory for all final figures
...
# ... (inside the loop 'for y_um, y_folder in y_positions:')

# (DELETE OR COMMENT OUT THESE LINES INSIDE THE LOOP)
# fig_dir = os.path.join(y_folder, "fig_AS_011_CC")
# os.makedirs(fig_dir, exist_ok=True)


# ... (Inside the loop, near the end of the LOAD section)

    # 2. UPDATE SAVE PATH FOR LOAD FIGURE
    fig_name_load = f"DC_Photocurrent_Y{y_um:04d}um_Quadrants_AD_thresholdX_RestricedRange_LOAD_results_inv"
    # fig_load.savefig(os.path.join(fig_dir, f"{fig_name_load}.png"), dpi=300, bbox_inches='tight') # ORIGINAL LINE
    fig_load.savefig(os.path.join(FINAL_FIG_DIR, f"{fig_name_load}.png"), dpi=300, bbox_inches='tight') # NEW LINE
    print(f"LOAD saved: {os.path.join(FINAL_FIG_DIR, f'{fig_name_load}.png')}") # Updated print statement
    plt.show()
    plt.close(fig_load)


# ... (Inside the loop, near the end of the GND section)

    # 2. UPDATE SAVE PATH FOR GND FIGURE
    fig_name_gnd = f"DC_Photocurrent_Y{y_um:04d}um_Quadrants_AD_thresholdX_RestricedRange_GND_results_inv"
    # fig_gnd.savefig(os.path.join(fig_dir, f"{fig_name_gnd}.png"), dpi=300, bbox_inches='tight') # ORIGINAL LINE
    fig_gnd.savefig(os.path.join(FINAL_FIG_DIR, f"{fig_name_gnd}.png"), dpi=300, bbox_inches='tight') # NEW LINE
    print(f"GND saved: {os.path.join(FINAL_FIG_DIR, f'{fig_name_gnd}.png')}") # Updated print statement
    plt.show()
    plt.close(fig_gnd)
'''