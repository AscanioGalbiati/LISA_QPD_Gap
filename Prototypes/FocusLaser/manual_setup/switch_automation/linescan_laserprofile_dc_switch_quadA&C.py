'''
This code processes data from a series of .pkl files, extracting the DC photocurrent and calculating the beam waist for quadA and quadC segments.

Compatible data: 
/Users/asca/Documents/University/Master Thesis/code/Data/final_data/20250910/VIGO17_333-2_QPD_0750_20_AP_011_CC_250910_LB1471C_quadA_and_C_manual_setup_fixedY_Z50mm_precise/Y3500um

Folder structure: 
final_data/QPDspecs_asca_date_YScan_lens_quad/Y...

Adapted for switch automation (using the Keithley 7001 switch sysyem) to handle multiple quadrants (quadA and quadC).
'''
...
# ====================================== DATA ==================================================== #
data_dirs = {
    'Y_A': "/Users/asca/Documents/University/Master Thesis/code/Data/final_data/20250910/VIGO17_333-2_QPD_0750_20_AP_011_CC_250910_LB1471C_quadA_and_C_manual_setup_fixedY_Z50mm_precise/Y3500um"
}

segns = ['quadA', 'quadC']  # Adapted for multiple quadrants
...
data_collection = {'Y_A': []}
procdata = {
    'Y_A': {segn: {'z_positions': [], 'beamwaist_dc': []} for segn in segns}
}
all_z_positions = set()
...
for y_pos, data_dir in data_dirs.items():
    ...
    for z_mm, pkl_file in z_positions_with_files:
        ...
        # Extract X position array (converted to um)
        xstart_um = data['global_params']['xstart_mm'] * 1000
        xstop_um = data['global_params']['xstop_mm'] * 1000
        xstep_um = data['global_params']['xstep_mm'] * 1000
        step = int((xstop_um - xstart_um) / xstep_um) + 1
        pos_array = np.linspace(xstart_um, xstop_um, step)
        
        # Update X-range for Y_A
        if y_pos == 'Y_A':
            if x_range['xstart_um'] is None or xstart_um > x_range['xstart_um']:
                x_range['xstart_um'] = xstart_um
            if x_range['xstop_um'] is None or xstop_um < x_range['xstop_um']:
                x_range['xstop_um'] = xstop_um
...
fig_figname = 'QPDGapScan_DCData_Y6100um_A_and_C' # ADJUST FOR Y_A VALUE
...
for y_pos in data_collection:
    ...
    for data_point in data_collection[y_pos]:
        ...
        alpha_val = 1.0 if 'quadA' in data_point and data_point['dc_curr_quadA'] is not None else 0.7
        for segn in segns:
            if f'dc_curr_{segn}' in data_point and data_point[f'dc_curr_{segn}'] is not None:
                dc_curr_filtered = data_point[f'dc_curr_{segn}'][mask]
                ax.plot(x_pos_filtered / 1000, dc_curr_filtered, 
                        linestyle=linestyle, color=line_color, alpha=alpha_val,
...
title = r'\textbf{DC Photocurrent (Y=6100\textmu{}m: quadA \& quadC)}' # ADJUST FOR Y_A VALUE
...
for y_pos in procdata:
    for segn in segns:
        min_waist = min(procdata[y_pos][segn]['beamwaist_dc']) if procdata[y_pos][segn]['beamwaist_dc'] else float('inf')
        print(f"Dataset {y_pos}, {segn}: Min waist: {min_waist:.4f} µm")
