...
json_config_fname = 'focus_laser_config_phasespace_xy_asca_manual_AngledScan.json'
...
if 'lta00' in focuslaser_jsonconfig.keys():
    ...
    lta00.write('1OR') 
    ypos = stage.conex.move(lta00, 1, float(focuslaser_jsonconfig['global_params']['along_start_mm']), 'abs')
...
if 'lta01' in focuslaser_jsonconfig.keys():
    ...
    # Force home movement. Comment out line if not want to go home. TODO, fix COEX homing.
    lta01.write('1OR') 
    xpos = stage.conex.move(lta01, 1, float(focuslaser_jsonconfig['global_params']['intercept_mm'] + focuslaser_jsonconfig['global_params']['slope'] * focuslaser_jsonconfig['global_params']['along_start_mm']), 'abs')

# -------------------- Configure scan parameters -------------------- #

# Extract parameters from config for angled scans
'''slope = focuslaser_jsonconfig['global_params']['slope']
intercept = focuslaser_jsonconfig['global_params']['intercept_mm']
norm = np.sqrt(1 + slope**2)
unit_perp_x = 1 / norm
unit_perp_y = -slope / norm'''
slope = focuslaser_jsonconfig['global_params']['slope']
intercept = focuslaser_jsonconfig['global_params']['intercept_mm']
perp_slope = -1 / slope
...
# Calculate the step in along-gap direction (reusing y params for along)
along_start = focuslaser_jsonconfig['global_params']['along_start_mm']
along_stop = focuslaser_jsonconfig['global_params']['along_stop_mm']
along_step_size = focuslaser_jsonconfig['global_params']['along_step_mm']
along_steps = int((along_stop - along_start) / along_step_size) + 1
along_array = np.linspace(along_start, along_stop, along_steps)

# Calculate the step in across-gap direction (u, perpendicular, reusing x params but as relative)
u_start = focuslaser_jsonconfig['global_params']['u_start_mm']
u_stop = focuslaser_jsonconfig['global_params']['u_stop_mm']
u_step_size = focuslaser_jsonconfig['global_params']['u_step_mm']
u_steps = int((u_stop - u_start) / u_step_size) + 1
u_array = np.linspace(u_start, u_stop, u_steps)

# -------------------- Save location config -------------------- #
# Data save root (updated to indicate angled scan)
save_dir_base = pathlib.Path('{}/{}/GapScanAngled/{}_{}_{}_{}_{}_{}_250926_LB1471C_quadA&D_manual_setup_AngledScan_Z52.5mm'.format(
    data_save_root, 
...
save_fname_str = "{}_{}_{}_{}_{}_{}_{}_QPDGapScan_KnifeEdge_250926_LB1471C_quadA&D_manual_setup_AngledScan_Z52.5mm".format(
    focuslaser_jsonconfig['dut']['batch'],
    focuslaser_jsonconfig['dut']['foundry'],
    focuslaser_jsonconfig['dut']['wafer'],
    focuslaser_jsonconfig['dut']['dut_type'],
...
for along in along_array:
    # Calculate center position for this along value (parameterized by y)
    x_center = intercept + slope * along
    y_center = along
    
    # Create subfolder for this along position
    ...
    data = {
        'rawdata': {
            ...
            'u_position': u_array,  # Add u for reference in analysis
            'quadA': {
                'dmm00_curr_amp': np.zeros((len(u_array), focuslaser_jsonconfig['global_params']['number_averages']))
            },
            'quadD': {
                'dmm00_curr_amp': np.zeros((len(u_array), focuslaser_jsonconfig['global_params']['number_averages']))
            }
    ...
    for idx, u in enumerate(u_array):
        ...
        try:
            ...
            print(f"along={along} mm, u={u} mm, DC photocurrent Quadrant D = {np.mean(temp_measurements_c):.7f} A")
        except Exception as e:
    ...
    plt.legend()
    plt.title(f'DC Photocurrent at along={along} mm')
    plt.xlabel('u Position along perpendicular (mm)')
    plt.ylabel('Photocurrent (A)')
    plt.show()

# -------------------- Cleanup -------------------- #
stage.conex.move(lta01, 1, float(focuslaser_jsonconfig['global_params']['intercept_mm'] + focuslaser_jsonconfig['global_params']['slope'] * focuslaser_jsonconfig['global_params']['along_start_mm']), 'abs')
stage.conex.move(lta00, 1, float(focuslaser_jsonconfig['global_params']['along_start_mm']), 'abs')
ser.write(b':open all\n')
time.sleep(0.5)
ser.close()
rm.close()
