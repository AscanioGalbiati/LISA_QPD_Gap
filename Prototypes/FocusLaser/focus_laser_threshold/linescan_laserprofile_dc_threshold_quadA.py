...
Folder structure: 
final_data/QPDspecs_date_thresholds_lens_quad/Y...
'''
...
# ====================================== DATA ==================================================== #
data_dirs = {
    'Y_A': "/Users/asca/Documents/University/Master Thesis/code/Data/VIGO17_NS089008_QPD_0750_20_AS_015_CC_asca_250617_focus_laser_thres_LB1471C_quadA/Y6100um"
}
...
fig10_figname = 'QPDGapScan_LaserProfile_DC_thres'
...
title10 = r'\textbf{Beam Profile (DC) with Thresholds}'
...
fig_figname = 'QPDGapScan_DCData_Y6100um_A_thres'  # Updated for Y_A value and thresholds
...
for y_pos in data_collection:
    ...
    for data_point in data_collection[y_pos]:
        ...
        if data_point['dc_curr_quadA'] is not None:
            ...
            line, = ax.plot(x_pos_filtered / 1000, dc_curr_filtered, 
                            linestyle=linestyle, color=line_color, alpha=1.0)
            # Add to legend only for the first and last Z positions
            z_pos = data_point['z_mm']
            if z_pos in (all_z_positions[0], all_z_positions[-1]):
                legend_handles.append(line)
                legend_labels.append(f'Z={z_pos:.2f} mm')

# Add step sizes to the legend
zstep_big = data['global_params']['zstep_big_um']
zstep_fine = data['global_params']['zstep_fine_um'] 
legend_handles.append(plt.Line2D([], [], color='none'))  # Empty handle for text
legend_labels.append(f'zstep_fine: {zstep_fine:.1f} µm')
legend_handles.append(plt.Line2D([], [], color='none'))  # Empty handle for text
legend_labels.append(f'zstep_big: {zstep_big:.1f} µm')
...
title = r'\textbf{DC Photocurrent (Y=6100\textmu{}m: quadA) with Thresholds}'
ax.set_title(title, fontsize=14, fontweight='bold', pad=10)
ax.grid(True, linestyle='--', alpha=0.6)
ax.tick_params(axis='both', which='major', labelsize=10, length=6, width=1.5, direction='in')
ax.legend(legend_handles, legend_labels, fontsize=12, loc='upper left', frameon=True, bbox_to_anchor=(0.02, 0.98))
